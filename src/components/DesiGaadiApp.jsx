...
export const DesiGaadiApp = () => {
  ...
  return (
    <div className={`min-h-screen w-full overflow-x-hidden transition-colors duration-300 ${isDarkMode ? 'bg-[#07090e] text-white' : 'bg-[#f4f6fb] text-slate-900 light-theme'}`}>
      ...
      {/* 1. TOP NAVBAR (100% Mobile Fluid & Responsive) */}
      <header className={`sticky top-0 z-40 w-full backdrop-blur-xl border-b px-3 sm:px-6 lg:px-8 py-2.5 sm:py-3 flex items-center justify-between gap-2 sm:gap-4 transition-colors duration-300 ${
        ...
        <div className="flex items-center gap-2 sm:gap-3 shrink-0 cursor-pointer">
          <div className="w-8 h-8 sm:w-10 sm:h-10 rounded-xl bg-amber-400 flex items-center justify-center text-slate-950 text-base sm:text-xl font-black shadow-md shadow-amber-500/20 shrink-0">
            🛺
          </div>
          <div className="min-w-0">
            <div className="flex items-center gap-1.5 sm:gap-2">
              <span className={`text-xs sm:text-base lg:text-lg font-black tracking-tight font-desi truncate ${isDarkMode ? 'text-white' : 'text-slate-950'}`}>
                DESI GAADI BEATS
              </span>
              <span className="text-[9px] sm:text-[10px] font-mono px-1.5 py-0.2 rounded bg-amber-400/20 text-amber-500 font-bold border border-amber-400/30 shrink-0">
                PRO
              </span>
            </div>
            <p className={`text-[9px] sm:text-[11px] font-mono tracking-wider hidden md:block truncate ${isDarkMode ? 'text-slate-400' : 'text-slate-500'}`}>
        ...
        {/* Right: Actions & Theme Switcher (Ultra-Compact on Mobile) */}
        <div className="flex items-center gap-1.5 sm:gap-2 shrink-0">
          
          {/* THEME TOGGLE BUTTON */}
          <button
            onClick={() => setIsDarkMode(!isDarkMode)}
            className={`flex items-center gap-1 px-2 sm:px-3 py-1.5 rounded-full text-[11px] sm:text-xs font-bold transition transform active:scale-95 border ${
              isDarkMode
                ? 'bg-amber-400 text-black border-amber-300 font-black shadow-md'
                : 'bg-slate-900 text-white border-slate-800 shadow-sm'
            }`}
            title={isDarkMode ? 'Switch to White Theme' : 'Switch to Black Theme'}
          >
            {isDarkMode ? (
              <>
              ...
              <>
                ...
                <span className="hidden md:inline">Black Mode</span>
              </>
            )}
          </button>
          ...
          <button
            onClick={handleHorn}
            className="flex items-center gap-1 px-2 sm:px-3 py-1.5 rounded-full bg-red-500/10 hover:bg-red-500/20 text-red-500 border border-red-500/30 text-[11px] sm:text-xs font-semibold transition transform active:scale-95"
            title="Blast Pressure Horn"
          >
            <Volume2 className="w-3.5 h-3.5 shrink-0" />
            <span className="hidden xs:inline">हॉर्न</span>
          </button>

          {/* Language Switcher */}
          <button
            onClick={() => setLanguage(language === 'hi' ? 'en' : 'hi')}
            className={`flex items-center gap-1 px-2 sm:px-2.5 py-1.5 rounded-full border text-[11px] sm:text-xs font-semibold transition ${
